...
public class GTAttackModifier extends GemType
{
    ...
    public void activate1(ItemStack stack, EntityPlayer player, Object... meta)
    {
        if (stack.getItem() instanceof GemAttackModifier) {
            ((GemAttackModifier) stack.getItem()).onEntityHit(stack, player, (EntityLivingBase) meta[0], (Stub<Float>) meta[1]);
        }
    }
    ...
    {
        if (stack.getItem() instanceof GemAttackModifier) {
            ((GemAttackModifier) stack.getItem()).onDealtDamage(stack, player, (EntityLivingBase) meta[0], (Float) meta[1]);
        }
    }
}
