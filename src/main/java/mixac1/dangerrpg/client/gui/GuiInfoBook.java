...
public class GuiInfoBook extends GuiScreen
{
	...
	public void setWorldAndResolution(Minecraft mc, int width, int height)
    {
    	...
    	int tmpX = offsetX + contentOffsetX;
    	int tmpY = offsetY + contentOffsetY + contentSize;;
	...
	public void drawScreen(int mouseX, int mouseY, float par3)
    {
        ...
        fontRendererObj.drawStringWithShadow(title, offsetX + (bookImageWidth - fontRendererObj.getStringWidth(title)) / 2,
        								  offsetY + (titleHeight - fontRendererObj.FONT_HEIGHT) / 2 + 2, 0xffffff);
    ...
    class SelectContentButton extends GuiButton
    {
		...
		public void drawButton(Minecraft mc, int par1, int par2)
        {
            if (this.visible) {
                ...
                else if (flag && this.enabled) {	// green
