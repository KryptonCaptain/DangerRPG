 ...
 */
public class FixIncorrectMotionHooks
{
    /**
    ...
    public static void sendLocationToAllClients(EntityTrackerEntry tracker, List list)
    {
        ...
        if (tracker.myEntity instanceof EntityItemFrame && tracker.ticks % 10 == 0) {
        ...
        else if (tracker.ticks % tracker.updateFrequency == 0 ||
                 tracker.myEntity.isAirBorne || tracker.myEntity.getDataWatcher().hasChanges()) {
            int i;
            int j;
            ...
            if (tracker.myEntity.ridingEntity == null) {
                ...
                if (tracker.ticks > 0 || tracker.myEntity instanceof EntityArrow) {
                    if (j1 >= -128 && j1 < 128 && k1 >= -128 && k1 < 128 && l1 >= -128 &&
                        l1 < 128 && tracker.ticksSinceLastForcedTeleport <= 400 && !tracker.ridingEntity) {
                        if (flag && flag1) {
                            object = new S14PacketEntity.S17PacketEntityLookMove(tracker.myEntity.getEntityId(),
                                                                                (byte)j1, (byte)k1, (byte)l1, (byte)l, (byte)i1);
                        }
                        else if (flag) {
                            object = new S14PacketEntity.S15PacketEntityRelMove(tracker.myEntity.getEntityId(),
                                                                                (byte)j1, (byte)k1, (byte)l1);
                        }
                        else if (flag1) {
                ...
                if (tracker.sendVelocityUpdates) {
                    ...
                    if (d4 > d3 * d3 || d4 > 0.0D && tracker.myEntity.motionX == 0.0D &&
                        tracker.myEntity.motionY == 0.0D && tracker.myEntity.motionZ == 0.0D) {
                        tracker.motionX = tracker.myEntity.motionX;
                        tracker.motionY = tracker.myEntity.motionY;
                        tracker.motionZ = tracker.myEntity.motionZ;
                         // T0D0: fix
                        tracker.func_151259_a(createVelocityPacket(tracker.myEntity.getEntityId(), tracker.motionX,
                                                                   tracker.motionY, tracker.motionZ));
                    }
                }
        ...
        if (tracker.myEntity.velocityChanged) {
             // T0D0: fix
            tracker.func_151261_b(createVelocityPacket(tracker.myEntity.getEntityId(), tracker.myEntity.motionX,
                                                       tracker.myEntity.motionY, tracker.myEntity.motionZ));
    ...
    public static void tryStartWachingThis(EntityTrackerEntry tracker, EntityPlayerMP player)
    {
        if (player != tracker.myEntity) {
            ...
            if (d0 >= (-tracker.blocksDistanceThreshold) && d0 <= tracker.blocksDistanceThreshold &&
                d1 >= (-tracker.blocksDistanceThreshold) && d1 <= tracker.blocksDistanceThreshold) {
                if (!tracker.trackingPlayers.contains(player) && (tracker.isPlayerWatchingThisChunk(player) ||
                                                                  tracker.myEntity.forceSpawn)) {
                    tracker.trackingPlayers.add(player);
                    Packet packet = tracker.func_151260_c();
                    player.playerNetServerHandler.sendPacket(packet);

                    if (!tracker.myEntity.getDataWatcher().getIsBlank()) {
                        player.playerNetServerHandler.sendPacket(new S1CPacketEntityMetadata(tracker.myEntity.getEntityId(),
                                                                                             tracker.myEntity.getDataWatcher(), true));
                    }

                    if (tracker.myEntity instanceof EntityLivingBase) {
                        ServersideAttributeMap map = (ServersideAttributeMap)((EntityLivingBase)tracker.myEntity).getAttributeMap();
                        Collection collection = map.getWatchedAttributes();

                        if (!collection.isEmpty()) {
                            player.playerNetServerHandler.sendPacket(new S20PacketEntityProperties(tracker.myEntity.getEntityId(), collection));
                        }
                    }
                    ...
                    if (posX != tracker.lastScaledXPosition || posY != tracker.lastScaledYPosition || posZ != tracker.lastScaledZPosition) {
                        FMLNetworkHandler.makeEntitySpawnAdjustment(tracker.myEntity, player, tracker.lastScaledXPosition,
                                                                    tracker.lastScaledYPosition, tracker.lastScaledZPosition);
                    }

                    if (tracker.sendVelocityUpdates && !(packet instanceof S0FPacketSpawnMob)) {
                         // T0D0: fix
                        player.playerNetServerHandler.sendPacket(createVelocityPacket(tracker.myEntity.getEntityId(), tracker.myEntity.motionX,
                                                                                      tracker.myEntity.motionY, tracker.myEntity.motionZ));
                    }

                    if (tracker.myEntity.ridingEntity != null) {
                        player.playerNetServerHandler.sendPacket(new S1BPacketEntityAttach(0, tracker.myEntity, tracker.myEntity.ridingEntity));
                    }

                    if (tracker.myEntity instanceof EntityLiving && ((EntityLiving)tracker.myEntity).getLeashedToEntity() != null) {
                        player.playerNetServerHandler.sendPacket(new S1BPacketEntityAttach(1, tracker.myEntity, ((EntityLiving)tracker.myEntity).getLeashedToEntity()));
                    }
                    ...
                    if (tracker.myEntity instanceof EntityLivingBase) {
                        for (int i = 0; i < 5; ++i) {
                            ...
                            if (stack != null) {
                                player.playerNetServerHandler.sendPacket(new S04PacketEntityEquipment(tracker.myEntity.getEntityId(), i, stack));
                            }
                        }
                    }
                    ...
                    if (tracker.myEntity instanceof EntityPlayer) {
                        ...
                        if (entityplayer.isPlayerSleeping()) {
                            player.playerNetServerHandler.sendPacket(new S0APacketUseBed(entityplayer, MathHelper.floor_double(tracker.myEntity.posX),
     ...
     */
    private static S12PacketEntityVelocity createVelocityPacket(int entityId, double motionX, double motionY, double motionZ)
    {
