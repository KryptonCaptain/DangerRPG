...
public class HookFixEntityAttributes
{
    ...
    public static void onImpact(EntityLargeFireball that, MovingObjectPosition mop)
    {
        if (!that.worldObj.isRemote) {
            ...
            that.worldObj.newExplosion((Entity)null, that.posX, that.posY, that.posZ, that.field_92057_e, true, that.worldObj.getGameRules().getGameRuleBooleanValue("mobGriefing"));
            that.setDead();
        }
    }
